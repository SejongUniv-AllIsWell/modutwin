...
const sourceBadge: Record<string, { label: string; className: string }> = {
  local:   { label: '로컬',     className: 'bg-[var(--bg-soft)] text-[var(--ink)]' },
  ...
  return (
    <div className={`flex items-center gap-2 px-2 py-1.5 rounded ${rowClass}`}>
      <button
        ...
        className={`p-1 rounded hover:bg-white/10 cursor-pointer ${visible ? 'text-[var(--ink)]' : 'text-[var(--muted)]'}`}
      >
      ...
      {selectable && onSelect ? (
        <button
          ...
          className={`flex-1 text-left text-xs truncate cursor-pointer ${visible ? 'text-[var(--ink)] hover:text-[var(--ink)]' : 'text-[var(--muted)] hover:text-[var(--ink-2)]'}`}
        >
          {name}
        </button>
      ) : (
        <span
          className={`flex-1 text-xs truncate ${visible ? 'text-[var(--ink)]' : 'text-[var(--muted)]'} ${active ? 'font-semibold' : ''}`}
          title={name}
        >
          {name}
        </span>
      ...
      {onRemove && (
        <button
          onClick={onRemove}
          title="제거"
          className="p-1 rounded text-[var(--muted)] hover:text-red-400 hover:bg-white/10 cursor-pointer"
        >
  ...
  return (
    <div className="w-72 bg-black/70 backdrop-blur-sm border border-white/10 rounded-lg shadow-lg select-none">
      <div className="px-3 py-2 border-b border-white/10 flex items-center gap-2">
        ...
        <span className="text-xs font-semibold text-[var(--ink)]">레이어</span>
      ...
      <div className="p-1.5 max-h-[60vh] overflow-y-auto">
        {empty && (
          <p className="text-[11px] text-[var(--muted)] px-2 py-3 text-center">
            아직 레이어가 없습니다.
          </p>
        )}
        {main && (
      ...
      {onAdditionalSelect && additional.some(it => it.source !== 'basemap') && (
        <div className="px-3 py-1.5 border-t border-white/10 text-[10px] text-[var(--muted)]">
