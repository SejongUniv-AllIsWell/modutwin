...
1. **Raw PLY 프레임** — `splatData.posX/Y/Z` 의 값. PLY 파일 그대로의 좌표. 메모리 안에서 다듬기 도중 안 변함.
2. **A' 프레임 (= raw + pendingRotation rotX/rotZ)** — `splatEntity` 의 local rotation 으로 적용 (`applyEntityRotation` in useRefineTool). 다듬기 화면에서 사용자가 보는 좌표 = Z-180 · A'. 텍스처 베이크의 source scene (`buildRotatedScene`) 도 이 프레임으로 회전된 사본을 사용 → 그래서 mesh corners 는 A' 프레임으로 산출됨. WallModal 의 `wallAngle` (Y축 회전) 은 화면 entity 에는 적용 안 되고 `surfacePlanesFromRoom` 의 평면 normal 정의 (-c, 0, -s) 형태로만 들어감.
3. **A'+Y 프레임 (= raw + pendingRotation + wallAngle Y)** — **문 설정 완료 시점 `commitRefinedToServer`** 에 PLY 와 mesh corners, doors corners 모두 이 프레임으로 베이크되어 서버에 저장. 재진입 시 splatEntity 에 default Z-180 만 부여해도 사용자가 정렬한 상태로 표시됨. ※ `saveRefined` 는 단순 mode transition 이고 실제 직렬화·회전 베이크·업로드는 `commitRefinedToServer` 에서 수행.
4. **World (렌더) 프레임** — 사용자가 화면에서 실제로 보는 좌표. `splatEntity.getWorldTransform()` 적용 결과. 카메라 (`cameraEntity.getPosition()`, `.forward`) 도 이 프레임.

프레임 사이 변환 규약:
- 다듬기 단계 화면: `A' → World` = `splatEntity` 에 **Z-180** + pendingRotation local rotation 부여 → 사용자 정렬 상태 표시. wallAngle 은 entity 에 안 들어가고 평면/메시 corners 정의에만 영향.
- 정합 단계 (또는 재진입) 화면: `A'+Y → World` = `splatEntity` 에 **Z-180** 만 부여 (PLY 가 이미 A'+Y 라 추가 회전 불필요).
- 메시 엔티티 (`createWallMeshEntity`, `createWallMeshFromPersisted`) 는 corners 가 A' (다듬기 도중) 또는 A'+Y (재진입) 프레임. **메시는 Z-180 만 부여하고 `app.root` 에 직접 부착** — `splatEntity` 의 자식으로 붙이면 pendingRotation 이 이중 적용됨.
- SOG/Supersplat 호환을 위해 baseline Z-180 (`setLocalEulerAngles(0, 0, 180)`) 컨벤션 채택. `(x, y, z) → (-x, -y, z)`.
